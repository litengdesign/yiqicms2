...
$link = @mysql_connect($cfg_db_host, $cfg_db_user, $cfg_db_pass) or die('不能连接到数据库,请确保用户名或密码正确 ');
    ...
    $db_selected = @mysql_select_db($cfg_db_name,$link) or die("指定数据库不存在");

$q = "SELECT * FROM yiqi_product where name like '%$_POST[keywords]%'"; //SQL询
$rs = mysql_query($q, $link); //连接
if(!$rs){die("<br/>链接失败");}
echo "<table class='inputform' cellpadding='1' cellspacing='1'>";
echo "<tr style='background:#f6f6f6;'><td class='w10'>选择</td><td class='w50'>产品标题</td><td>所属分类</td><td class='w10'>发布状态</td><td class='w20'>相关操作</td></tr>";

while($row = mysql_fetch_row($rs)) echo "<tr><td><input id='slt3' type='checkbox' name='chk[]' value='3' /></td><td><a href='../product.php?name=$row[7]' target='_blank'>$row[1]</a>
</td><td>$row[2]</td><td>$row[13]</td><td  class='w20'><a href='product-edit.php?pid=$article$row[0]'>修改</a>
&nbsp;&nbsp;&nbsp;
<a href='pdelete.php?pid=$article$row[0]'  id='d_pro' onclick='return confirm(确定删除吗？);'>删除</a>
</td></tr>"; //
echo "</table>";
echo "<script>
  $('#d_pro').click(function(){
  	if(confirm('确定删除吗？')){
  		return true;
  	}
  	else{
