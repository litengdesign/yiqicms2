...
if($action == "istop")
{
    ...
    if(count($idarr) > 0)
    {
        foreach($idarr as $id)
        {
            if(is_numeric($id))
            {
                $sql = "UPDATE yiqi_article set istop='true' WHERE aid = '$id' ";

            
                $yiqi_db->query(CheckSql($sql));
            }
        }
        echo "<script>alert('已经设置为置顶文章');history.go(-1)</script>";
    }
}
if($action == "notop")
{
    ...
    if(count($idarr) > 0)
    {
        foreach($idarr as $id)
        {
            if(is_numeric($id))
            {
                $sql = "UPDATE yiqi_article set istop='false' WHERE aid = '$id' ";

            
                $yiqi_db->query(CheckSql($sql));
            }
        }
        echo "<script>alert('取消置顶成功');history.go(-1)</script>";
    }
}
?>
<?php
...
if(count($articlelist)>0)
{
    foreach($articlelist as $article)
    {
        ...
        {
           $istop="<span title='置顶' style='color:red'>  置顶</span>";

        }
        else{
            $istop="";
        }
         if($article->newsthumb !=NULL)
        {
           $newsthumb="<span title='图' style='color:blue'>  图</span>";
        ...
        echo "<tr>".
			...
			"<td><a href=\"../article.php?name=".urlencode($article->filename)."\" target=\"_blank\">$article->title</a>$istop $newsthumb</td>".
...
<div class="clear">&nbsp;</div>
<div class="fl" style="text-indent:16px;"><input id="slt" type="checkbox"/>&nbsp;&nbsp;<select name="action">
