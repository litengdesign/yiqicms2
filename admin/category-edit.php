...
if($action == "save")
{
    ...
    $categorycatethumb = $_POST["categorycatethumb"];
    ...
    if(empty($categorycatethumb))
    {
        $categorycatethumb = $catinfo->catethumb;
    }
	...
	}else if (isset($_POST['categorycatethumb'])&&$_POST['categorycatethumb']!=$catinfo->thumb)
	{
		$categorycatethumb = $_POST['categorycatethumb'];
	}
    ...
    $sql = "UPDATE yiqi_category SET name = '$categoryname',seotitle='$categoryseotitle',seokeywords='$categoryseokeywords',seodescription='$categoryseodescription',catethumb='$categorycatethumb',description='$categorydescription',filename = '$categoryfilename',templets = '$categorytemplets',takenumber = '$categorytakenumber' WHERE cid = '$cid'";
...
<tr><td class="label">分类图片</td><td class="input"><?php 
    if($catinfo->catethumb == "-")
    {
        echo '您未上传缩略图,您可以上传图片 <div id="ptinfo"><input class="upfile txt" type="file" style="width:280px;" name="categorycatethumb" /> 或者 <a href="javascript:void(0);" onclick="setinput();" style="color:#0000cc;">输入地址</a></div>';
    }
    else
    {
        echo '您已经上传了缩略图,如果需要修改，请重新上传图片 <div id="ptinfo"><input class="upfile txt" type="file" style="width:280px;" name="categorycatethumb" /> 或者 <a href="javascript:void(0);" onclick="setinput();" style="color:#0000cc;">输入地址</a></div>';
		echo '图片预览:<br /><a href="'.$catinfo->catethumb.'" target="_blank"><img src="'.$catinfo->catethumb.'" style="width:100px;height:100px;" ></a>';
    }
?></td></tr>
