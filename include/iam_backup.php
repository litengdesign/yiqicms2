...
/**
		 ...
		 */ class iam_backup
	{
		...
		function _get_def( $tablename )
		{
			...
			$def .= "#------------------------------------------" . $this->newline;
			$def .= "DROP TABLE  IF EXISTS $tablename;$this->newline#<--break-->$this->newline" . $this->newline;
			...
			$def .= "$this->newline) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;$this->newline#<--break-->$this->newline";
		...
		function _get_content( $tablename )
		{
			...
			while ( $row = @mysql_fetch_row($result) )
			{
				...
				$insert .= ");$this->newline#<--break-->$this->newline";
				$content .= $insert;
			}
