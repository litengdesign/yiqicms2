...
{    
	...
	//置顶新闻
	 function TakeArticleTopListByName($name="",$skip=0,$take=10,$orderby="adddate desc",$all=false)
	{
	    global $yiqi_db;
	    $categorydata = new Category();	    
    	$exist = $categorydata->ExistFilename($name);
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_article where cid in ($cids) and istop='true' and newsthumb !='NULL' order by $orderby limit $skip,$take "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_article where cid in ($cids) and istop='true' and newsthumb !='NULL' and adddate <= '%s' order by $orderby limit $skip,$take ",date("Y-m-d H:i:s"))));
			}
		}
		else
		{
		...
		//最新新闻
	 function TakeArticleNewListByName($name="",$skip=0,$take=10,$orderby="adddate desc",$all=false)
	{
	    global $yiqi_db;
	    $categorydata = new Category();	    
    	$exist = $categorydata->ExistFilename($name);
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_article where cid in ($cids) and istop='false' and newsthumb !='NULL' order by $orderby limit $skip,$take "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_article where cid in ($cids) and istop='false' and newsthumb !='NULL' and adddate <= '%s' order by $orderby limit $skip,$take ",date("Y-m-d H:i:s"))));
			}
		}
		else
		{
