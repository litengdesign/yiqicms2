...
class Product
{
	...
	//获取推荐产品
	function TakeisgoodProductList($cid=0,$skip=0,$take=10,$orderby="adddate desc",$all=false)
	{
	    global $yiqi_db;
	    $categorydata = new Category();	    
    	$exist = $categorydata->ExistCategory($cid);
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) and isgood='true' order by $orderby limit $skip,$take "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_product where cid in ($cids) and isgood='true' and adddate <= '%s' order by $orderby limit $skip,$take ",date("Y-m-d H:i:s"))));
			}
		}
		else
		{
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where isgood='true' order by $orderby limit $skip,$take"));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_product where isgood='true' and adddate <= '%s' order by $orderby limit $skip,$take",date("Y-m-d H:i:s"))));
			}
		}
	}
	//end
	//获取最新产品
	function TakeisnewProductList($cid=0,$skip=0,$take=10,$orderby="adddate desc",$all=false)
	{
	    global $yiqi_db;
	    $categorydata = new Category();	    
    	$exist = $categorydata->ExistCategory($cid);
		if($exist == 1)
		{
			...
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where cid in ($cids) and isnew='true' order by $orderby limit $skip,$take "));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_product where cid in ($cids) and isnew='true' and adddate <= '%s' order by $orderby limit $skip,$take ",date("Y-m-d H:i:s"))));
			}
		}
		else
		{
			if($all)
			{
				return $yiqi_db->get_results(CheckSql("select * from yiqi_product where isnew='true' order by $orderby limit $skip,$take"));
			}
			else
			{
				return $yiqi_db->get_results(CheckSql(sprintf("select * from yiqi_product where isnew='true' and adddate <= '%s' order by $orderby limit $skip,$take",date("Y-m-d H:i:s"))));
			}
		}
	}
	//end
 ...
 //搜索代码
        function SearchCategoryList($keywords)
        {
                global $yiqi_db;
                $sql = "select * from yiqi_product where name like '%$keywords%' or content like '%$keywords%'";
